...
def add_syscall(code: x86.Program, type: Interupt, ebx: Argument, ecx: Argument, edx: Argument):
...
def add_syscall_fwrite(code: x86.Program, char_pointer: int | x86.Register | x86.Label, size: int | x86.Register | x86.Label, file_descriptor: int | x86.Register | x86.Label):
    add_syscall(code, Interupt.WRITE, file_descriptor, char_pointer, size)
    
def add_syscall_exit(code: x86.Program, exit_code: Argument):
