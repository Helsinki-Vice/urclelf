"This module takes URCL source code and tokenizes it."
import enum
...
class TokenType(enum.Enum):
    ...
    MACRO = "MACRO"
    PORT = "PORT"
    HEADER_INEQUALITY = "HEADER_INEQUALITY"
...
TokenValue = Union[str, int, primitives.GeneralRegister | primitives.HeapAddress | primitives.Label, None]
...
class TokenParseResult:
    ...
    @classmethod
    def success(cls, type: TokenType, value: TokenValue, chars_consumed: int, source: str):
        return TokenParseResult(Token(type, 1, 1, value, source), chars_consumed)
...
def extract_label_token(source: str) -> TokenParseResult:
    ...
    return TokenParseResult.success(TokenType.LABEL, primitives.Label(source[1:index]), index, source[:index])
...
def extract_space_token(source: str) -> TokenParseResult:
    ...
    while index < len(source):
        ...
        index += 1
    if index:
        return TokenParseResult.success(TokenType.WHITESPACE, None, index, source[:index])
...
def extract_relative_jump_token(source: str) -> TokenParseResult:
    ...
    return TokenParseResult.success(TokenType.RELATIVE_JUMP, token_value, index, source[:index])
...
def extract_register_token(source: str) -> TokenParseResult:
    ...
    if index == 1:
        return TokenParseResult.miss()
    try:
        token_value = primitives.GeneralRegister(int(source[1:index], base=0))
    except ValueError:
        return TokenParseResult(f"Malformed register '{source[1:index]}'", 0)
            
    return TokenParseResult.success(TokenType.GENERAL_REGISTER, token_value, index, source[:index])
...
def extract_memory_address_token(source: str) -> TokenParseResult:
    ...
    if index == 1:
        return TokenParseResult.miss()
    try:
        token_value = primitives.HeapAddress(int(source[1:index], base=0))
    except ValueError:
        return TokenParseResult(f"Malformed memory address '{source[1:index]}'", 0)
            
    return TokenParseResult.success(TokenType.HEAP_ADDRESS, token_value, index, source[:index])
...
def extract_integer_token(source: str) -> TokenParseResult:
    ...
    return TokenParseResult.success(TokenType.INTEGER, token_value, index, source[:index])
...
def extract_string_token(source: str) -> TokenParseResult:
    ...
    return TokenParseResult.success(TokenType.STRING, string_value, index, source[:index])
...
def extract_character_token(source: str) -> TokenParseResult:
    ...
    return TokenParseResult.success(TokenType.CHARACTER, string_value, index, source[:index])
...
def extract_identifier_token(source: str) -> TokenParseResult:
    ...
    return TokenParseResult.success(TokenType.IDENTIFIER, source[:index], index, source[:index])
...
def extract_macro_token(source: str) -> TokenParseResult:
    ...
    return TokenParseResult.success(TokenType.MACRO, source[1:index], index, source[:index])
...
def extract_port_token(source: str) -> TokenParseResult:
    ...
    return TokenParseResult.success(TokenType.PORT, source[1:index], index, source[:index])

def extract_header_inequality_token(source: str) -> TokenParseResult:

    for inequality in ["==", "<=", ">="]:
        if source.startswith(inequality):
            return TokenParseResult.success(TokenType.HEADER_INEQUALITY, source[:2], 2, source[:2])
        ...
        while self.index < len(self.source):
            for token_extraction_function in TOKEN_EXTRACTION_FUNCTIONS:
                ...
                if result.data.type != TokenType.WHITESPACE:
                    tokens.append(Token(result.data.type, self.line_number, self.column_number, result.data.value, result.data.source))
